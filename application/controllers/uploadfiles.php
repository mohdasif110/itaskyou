...
class Uploadfiles extends Admin_Controller {
	...
	if(isset($_REQUEST['userID'])){
		...
		switch($attachementType){
			...
			$mediaContent			=	json_decode($mediaContent);
			...
			$Data					=	array(
												...
												'mediaContent'=>json_encode($mediaContent),
												'mediaURL'=>'',
		...
		if($attachmentID==0){
			...
			echo json_encode(array("Contact"=>$mediaContent,'attachmentID'=>0,'attachementType'=>$attachementType,'attachementFor'=>$attachementFor,'status'=>'false'));
			
		}else{
			
			echo json_encode(array("Contact"=>$mediaContent,'attachmentID'=>$attachmentID,'attachementType'=>$attachementType,	'attachementFor'=>$attachementFor,'status'=>'true'));
		}
		die();
		break;	
		...
		if($attachmentID==0){
			...
			echo json_encode(array("Location"=>$mediaContent,'attachmentID'=>0,'attachementType'=>$attachementType,'attachementFor'=>$attachementFor,'status'=>'false'));
			
		}else{
			
			echo json_encode(array("Location"=>$mediaContent,'attachmentID'=>$attachmentID,'attachementType'=>$attachementType,	'attachementFor'=>$attachementFor,'status'=>'true'));
		}
		die();
		break;
		default: 
