...
if($tabval=='allnew'){
			...
			foreach($DataTask as $key=>$dval){
				
				if($dval->taskIN=='Group'){
					
					$grouDetail =$this->group_model->get_group_detail_at_home($dval->funID);
					
					if($grouDetail->status=='1'){
					
						//unset($dval->taskIN);	
						$tempData["G".$dval->funID]['listType']				=	'group';
						$tempData["G".$dval->funID]['groupID']				=	$grouDetail->groupID;
						$tempData["G".$dval->funID]['groupName']			=	$grouDetail->groupName;
						$tempData["G".$dval->funID]['firstName']			=	$grouDetail->firstName;
						$tempData["G".$dval->funID]['lastName']				=	$grouDetail->lastName;
						$tempData["G".$dval->funID]['groupPhoto']			=	$grouDetail->groupPhoto;
				
					if($dval->actionType!='Group'){
						
						$tempData["G".$dval->funID]['groupTask'][]			=	$dval;
					}
			...
			if($dval->taskIN=='MOM'){
				//unset($dval->taskIN);	
				$tempData['Mom'.$dval->funID]['listType']									=	'mom';
				$tempData['Mom'.$dval->funID]['momTask'][]									=	 $dval;
			}
			
			if($dval->taskIN=='Task'){
				//unset($dval->taskIN);
				$dval->listType			=	'Task';		
				$tempData["T".$dval->taskID]			=	 $dval;
			}
		}
		...
		echo json_encode(array("action"=>'success','auth'=>'true','message'=>'success' ,'dataValue'=>$tempData));
		die();
	   ...
	   if(isset($_REQUEST['groupID'])){
		...
		if(isset($_REQUEST['participantID'])){
			...
			if($_REQUEST['participantID']!=''){
				...
				echo  json_encode(array('action'=>'error', 'message'=>'Please send participantID.'));
				die();
			}
	
		}else{
			
			echo  json_encode(array('action'=>'error', 'message'=>'Please send a participantID to the server.'));
			die();
		}
		
		$this->add_group_members($participantID,$groupID,$userID);	
...
// Group home screen.. 
 ...
 //Group home screen.. 
	...
	//End...
	function delete_team_from_group(){
	...
	function  marked_group_admin()
	{
		...
		if(isset($_REQUEST['mark_as_admin_user_ids'])){
		
			if($_REQUEST['mark_as_admin_user_ids']!=''){
					
					$mark_as_admin_user_ids 			=	$_REQUEST['mark_as_admin_user_ids'];
