			...
			if($dval->taskIN=='Group'){
				...
				$grouDetail =$this->group_model->get_group_detail_at_home($dval->funID);
					...
					if($grouDetail->status=='1'){
						...
						unset($dval->taskIN);	
						$tempData[$dval->funID]['listType']				=	'group';
						$tempData[$dval->funID]['groupID']				=	$grouDetail->groupID;
						$tempData[$dval->funID]['groupName']			=	$grouDetail->groupName;
						$tempData[$dval->funID]['firstName']			=	$grouDetail->firstName;
						$tempData[$dval->funID]['lastName']				=	$grouDetail->lastName;
						$tempData[$dval->funID]['groupPhoto']			=	$grouDetail->groupPhoto;
					
					if($dval->actionType!='Group'){
						
						$tempData[$dval->funID]['groupTask'][]			=	$dval;
					}
					
				}	
			
			}elseif($dval->taskIN=='MOM'){
			
			
				unset($dval->taskIN);	
				$tempData[$dval->funID]['listType']				=	'mom';
				$tempData[$key]									=	 $dval;
