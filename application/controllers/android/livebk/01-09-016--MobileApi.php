...
class MobileApi extends CI_Controller
{
	...
	function get_otp_number(){
		//echo random_string('alnum',4);
	}
	...
	function  check_contact_number()
	{
			...
			if(count($getData)>0)
			{
				...
				$flagsave		=	$this->db->update($this->db->dbprefix('api_users') , array('deviceID'=>$deviceID,'apiKey'=>$apiKey));
				...
				$flagsave		=	$this->db->insert($this->db->dbprefix('api_users') , array('status'=>'0' , 'countryPhoneCode'=>$country_code,'contactNo'=>$phone_number,'otp'=>$otp_code,'deviceID'=>$deviceID,'apiKey'=>$apiKey));
