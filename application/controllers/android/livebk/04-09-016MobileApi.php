...
class MobileApi extends CI_Controller
{
	...
	function upate_profile()
	{
		...
		if(isset($_GET['subIndustoryType']) && $_GET['subIndustoryType']!=''){
		
			$subIndustoryType		=	$_GET['subIndustoryType'];
		
		}else{
			
			$temp_error['subIndustoryType']				= "Please select sub industory type.";	
		}
		...
		if($numrows==0){
			...
			echo  json_encode(array('action'=>'error', 'message'=>'Phone number is not available in dataabse.'));
			return false;
		}
		...
		if($this->db->update($this->db->dbprefix('api_users'),  $data))
		{
			...
			//$dataGamming	=	$this->db->select('firstName,lastName,emailID,industryType,subIndustoryType,nationality,countryPhoneCode,contactNo,userPhoto')->from($this->db->dbprefix('api_users'))->where(array('contactNo'=>$phone_number))->get()->row();
			...
			//echo  json_encode(array('action'=>'success', 'message'=>'Profile has been  updated successfully'));
