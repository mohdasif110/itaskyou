...
class MobileApi extends CI_Controller
{
	...
	*/	
	...
	function chnage_task_params_value()
	{
		...
		if($this->db->update($this->db->dbprefix('api_tasks'),$myField))
		{
			...
			if($action!=''){	
				...
				$Data			=	array('action'=>'update_task','message'=>$action,'taskName'=>$TaskDetail->taskName,'taskID'=>$taskID);
				//Send Push Notification  to  the server
	...
	function get_task_history()
	{
		...
		$data	    = $this->db->select('taskID,createdDate,title,actions')
			...
			echo json_encode(array('action'=>'success',"logData"=>$data));	
