...
Class User extends CI_Model
{
	...
	function get_api_users(){
		...
		$dataGamming	=	$this->db->select('U.firstName, U.status,U.userID,U.lastName,U.emailID,ITM.type as industryType,SITM.subType as subIndustryTypeID,U.countryPhoneCode,U.contactNo,U.userPhoto,U.AppAccessFlag')
			...
			if(count($dataGamming)>0){
				...
				foreach($dataGamming as $val){
					
					$val->AppAccessFlag		=		json_decode($val->AppAccessFlag);
					$tempArr[]		= $val;	
				}
			}
	...
	function get_user_info($conditionArr){
			...
			$dataGamming	=	$this->db->select('U.userID,U.deviceID,U.firstName,U.lastName,U.emailID,ITM.type as industryType,SITM.subType as subIndustryTypeID,U.countryPhoneCode,U.contactNo,U.userPhoto')
			->from($this->db->dbprefix('api_users AS U'))
			->join($this->db->dbprefix('api_industory_type_master ITM'),'ITM.typeID=U.industryType')
			->join($this->db->dbprefix('api_sub_industory_type_master SITM'),'SITM.subTypeID=U.subIndustoryType', 'left')
			->where($conditionArr)
	...
	//
	function get_multiple_active_users_token($userArr	=	array())
	{
		
		$dataGamming	=	$this->db->select('U.appKey')
			->from($this->db->dbprefix('api_users AS U'))
			->join($this->db->dbprefix('api_industory_type_master ITM'),'ITM.typeID=U.industryType')
			->join($this->db->dbprefix('api_sub_industory_type_master SITM'),'SITM.subTypeID=U.subIndustoryType', 'left')
		    ->order_by('U.firstName', 'asc')
	...
	//function authenticat_device($userID=null,$deviceID=null){
